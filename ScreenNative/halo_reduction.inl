...
cbuffer HC : register(b0) { uint gW; uint gH; int gDirX; int gDirY; int gRadius; int gStep; uint gMode; float gSigmaR; };
...
// Dc = D - H * box3(D - Tbil). t0 nr_in, t1 nr_out, t2 Tbil.
...
RWTexture2D<float4> gDst : register(u0);
cbuffer HC : register(b0) { uint gW; uint gH; int gDirX; int gDirY; int gRadius; int gStep; uint gMode; float gHalo; };
[numthreads(8, 8, 1)]
void CSMain(uint3 id : SV_DispatchThreadID)
{
    if (id.x >= gW || id.y >= gH) return;
    ...
    gDst[p] = float4(d - gHalo * (e / 9.0f), 0.0f);
}
)hlsl";
...
static ID3D12DescriptorHeap *g_halo_heap;          // three sets of 3 SRV + 1 UAV
static ID3D12Resource       *g_halo_bound[3][4];
...
static bool EnsureHaloPipeline()
{
    ...
    Log("[halo] pipeline ready (cross-bilateral r=%d step=%d, sigma_r=%.2f)", kHaloRadius, kHaloStep, kHaloSigmaR);
    return true;
}
...
static void BindHaloSet(UINT slot, ID3D12Resource *t0, ID3D12Resource *t1, ID3D12Resource *t2,
                        ID3D12Resource *u0)
{
    ID3D12Resource *want[4] = { t0, t1, t2, u0 };
    ...
    cpu.ptr += static_cast<SIZE_T>(slot) * 4 * stride;
    for (int i = 0; i < 3; ++i)
    {
        D3D12_SHADER_RESOURCE_VIEW_DESC sd = {};
        sd.Format = want[i]->GetDesc().Format;
...
static void HaloFilter(ID3D12Resource *nr_in, ID3D12Resource *nr_out, UINT w, UINT hgt, float halo)
{
    ...
    BindHaloSet(1, nr_in, g_halo_t.th, nr_in, g_halo_t.tb);
    ...
    BindHaloSet(2, nr_in, nr_out, g_halo_t.tb, g_halo_t.dc);
