...
// failed evaluate, every time (1.3.2).
static constexpr unsigned kMaxPasses = 4;
...
static bool EnsureMultiPass(VideoState& v)
{
    ...
    // Four since 1.3.2: people with cards that can afford it asked for it.
    // Each pass is another whole run of the network, so this is a straight
    // multiplier on the frame time - the UI says so beside the slider.
