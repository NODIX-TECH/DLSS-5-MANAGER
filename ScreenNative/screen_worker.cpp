...
// bit 7: a recording is running. Not one frame's request for pixels but the
// state: while it is set, FLOW MFG's presenter writes every frame it shows
// into the pixel section, so the recording holds the frames in between and
// not just the network's rate. It reads a whole frame back over PCIe, which
// is why it waits for this rather than doing it always.
static constexpr uint32_t FRAME_FLAG_RECORDING = 0x80u;
...
// Which source the last frame came from. Frame generation - DLSS 4.5 FG and
// FLOW MFG alike - used to treat every bypass frame as a reset, so with the
// network off the switch appeared to do nothing: the presenter kept running
// and never interpolated. Only the switch between the neural frame and the
// raw capture is a real discontinuity.
static bool g_last_bypass = false;
static bool EnsurePresentFormat(bool hdr, bool pq = false);
static void CloseHdrResources();

// ---------------------------------------------------------------------------
// Sharpness, edge smoothing and FLOW MFG - three LIVE FLOW settings that
// travel together in the resize command's `profile` field. That field has
// been sent as a fixed zero since the 310.8.0 runtime proved it dead (the
// wire kept its shape), so no client that predates them can set bit 31:
//   bit 31      the settings below are present
//   bits 0..7   sharpness, 0..255 for 0..1          (post_finish.inl)
//   bits 8..15  edge smoothing, 0..255 for 0..1      (post_finish.inl)
//   bits 16..19 FLOW MFG frames in between, 0 off .. 5 = x6 (flow_mfg.inl)
// The environment - NS_SHARPNESS, NS_EDGE_SMOOTH, NS_FLOW_MFG (the
// multiplier) - decides the first frames, before any resize has arrived.
// ---------------------------------------------------------------------------
static constexpr uint32_t EXTRAS_PRESENT = 0x80000000u;
static float g_sharpness = -1.0f;     // -1: not read from the environment yet
static float g_edge_smooth = -1.0f;
static int   g_flow_count = -1;

static float EnvUnit(const char *name)
{
    char buf[16] = {};
    const DWORD got = GetEnvironmentVariableA(name, buf, sizeof(buf));
    const float f = (got > 0 && got < sizeof(buf)) ? static_cast<float>(atof(buf)) : 0.0f;
    return f > 0.0f ? (f < 1.0f ? f : 1.0f) : 0.0f;
}

static float CurrentSharpness()
{
    if (g_sharpness < 0.0f) g_sharpness = EnvUnit("NS_SHARPNESS");
    return g_sharpness;
}

static float CurrentEdgeSmooth()
{
    if (g_edge_smooth < 0.0f) g_edge_smooth = EnvUnit("NS_EDGE_SMOOTH");
    return g_edge_smooth;
}

static unsigned CurrentFlowCount()
{
    if (g_flow_count < 0)
    {
        char buf[8] = {};
        const DWORD got = GetEnvironmentVariableA("NS_FLOW_MFG", buf, sizeof(buf));
        const int multiplier = (got > 0 && got < sizeof(buf)) ? atoi(buf) : 0;
        g_flow_count = (multiplier >= 2 && multiplier <= 6) ? multiplier - 1 : 0;
    }
    return static_cast<unsigned>(g_flow_count);
}

static void ExtrasFromResize(uint32_t word)
{
    if ((word & EXTRAS_PRESENT) == 0) return;
    const float sharp = static_cast<float>(word & 0xFFu) / 255.0f;
    const float edge = static_cast<float>((word >> 8) & 0xFFu) / 255.0f;
    const int flow = static_cast<int>(std::min(5u, (word >> 16) & 0xFu));
    if (sharp != CurrentSharpness() || edge != CurrentEdgeSmooth())
        Log("[finish] sharpness %.2f, edge smoothing %.2f", sharp, edge);
    if (flow != static_cast<int>(CurrentFlowCount()))
        Log(flow > 0 ? "[flow] FLOW MFG x%d requested" : "[flow] off", flow + 1);
    g_sharpness = sharp;
    g_edge_smooth = edge;
    g_flow_count = flow;
}

// FLOW MFG owns the output window while it is on: see flow_mfg.inl.
static bool g_present_on_flow_queue = false;   // the swap chain lives on FLOW's queue
static UINT64 g_flow_present_fence = 0;        // FlowPresent's token, like g_fg_present_fence
static HANDLE g_flow_waitable = nullptr;       // owned until this swap chain closes
...
#include "capture_shortcuts.inl"

static LRESULT CALLBACK PresentWndProc(HWND w, UINT m, WPARAM wp, LPARAM lp)
{
    switch (m)
    {
    case WM_APP + 42: BeginSnapshot(15000); return 0;
    case WM_APP + 43: EndSnapshot(); return 0;
    case WM_TIMER: if (wp == kSnapshotTimer) { EndSnapshot(); return 0; } break;
...
static DWORD WINAPI PresentWindowThread(LPVOID)
{
    ...
    wc.lpszClassName = L"DLSS5ManagerScreenPresent";
    ...
    g_present_hwnd = CreateWindowExW(
        ...
        wc.lpszClassName, L"DLSS 5 MANAGER Screen", WS_POPUP,
    ...
    g_snapshot_hook = SetWindowsHookExW(WH_KEYBOARD_LL, SnapshotKeys, GetModuleHandleW(nullptr), 0);
    if (!g_snapshot_hook) Log("[snapshot] shortcut hook unavailable %lu", GetLastError());
    MSG msg;
    while (GetMessageW(&msg, nullptr, 0, 0) > 0) { TranslateMessage(&msg); DispatchMessageW(&msg); }
    if (g_snapshot_hook) UnhookWindowsHookEx(g_snapshot_hook);
    g_snapshot_hook = nullptr;
...
{
    CloseFlow();
    if (g_flow_waitable) { CloseHandle(g_flow_waitable); g_flow_waitable = nullptr; }
    CloseFgResources();
    if (g_present_swap != nullptr) { g_present_swap->Release(); g_present_swap = nullptr; }
    g_present_on_flow_queue = false;
    ...
    // And FindWindowW(L"DLSS5ManagerScreenPresent", L"DLSS 5 MANAGER Screen") - how the HUD
...
static bool OpenPresent(UINT width, UINT height, uint32_t flags)
{
    ...
    if (FlowWanted()) sd.Flags |= DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT;
    IDXGISwapChain1 *sc1 = nullptr;
    // The swapchain must be created on the very queue that will write the back
    // buffer, which is the same queue NGX submits on - except while FLOW MFG
    // is on: its presenter writes and presents from a queue of its own, so
    // the frames in between never wait behind the network (flow_mfg.inl).
    ID3D12CommandQueue *swap_queue = FlowSwapQueue();
    hr = factory->CreateSwapChainForHwnd(swap_queue, g_present_hwnd, &sd, nullptr, nullptr, &sc1);
    g_present_on_flow_queue = SUCCEEDED(hr) && swap_queue != h.queue;
    ...
    Log("[present] overlay %ux%u ready (flip-discard, layered click-through%s%s)",
        width, height, g_present_capturable ? ", CAPTURABLE" : ", hidden from capture",
        g_present_on_flow_queue ? ", FLOW MFG queue" : "");
    return true;
}
...
// How much of the captured window other windows cover right now, 0..1.
//
// Only windows above it in the z-order can hide it, so those are the ones
// walked. Left out: the picture itself, the LIVE FLOW panel, anything
// invisible, minimised or cloaked (a suspended store app, another virtual
// desktop), and click-through transparent windows - NVIDIA's full-screen
// overlay and its kind cover nothing anyone can see.
static float CapturedWindowCovered(const RECT &target)
{
    const LONGLONG area = static_cast<LONGLONG>(target.right - target.left) *
                          static_cast<LONGLONG>(target.bottom - target.top);
    if (area <= 0) return 0.0f;
    LONGLONG covered = 0;
    int guard = 0;
    for (HWND w = GetWindow(g_wgc_hwnd, GW_HWNDPREV); w != nullptr && guard < 512;
         w = GetWindow(w, GW_HWNDPREV), ++guard)
    {
        if (w == g_present_hwnd || !IsWindowVisible(w) || IsIconic(w)) continue;
        BOOL cloaked = FALSE;
        ...
        if ((ex & WS_EX_TRANSPARENT) != 0 && (ex & WS_EX_LAYERED) != 0) continue;
        wchar_t title[80] = {};
        if (GetWindowTextW(w, title, 80) && wcscmp(title, L"DLSS 5 MANAGER Screen overlay") == 0) continue;
        RECT r = {};
        if (FAILED(DwmGetWindowAttribute(w, DWMWA_EXTENDED_FRAME_BOUNDS, &r, sizeof(r))) &&
            !GetWindowRect(w, &r))
            continue;
        RECT i = {};
        if (!IntersectRect(&i, &r, &target)) continue;
        covered += static_cast<LONGLONG>(i.right - i.left) * static_cast<LONGLONG>(i.bottom - i.top);
        if (covered >= area) break;
    }
    return static_cast<float>(static_cast<double>(covered) / static_cast<double>(area));
}

// Whether the picture belongs over the window at this moment: not while it
// is minimised, and not while a quarter or more of it is under something
// else - a captured app must never pin its image over an app the user put
// in front of it.
//
// It used to be "while it has the focus", and focus is the wrong question:
// a click on the manager beside the game, on the LIVE FLOW panel or on the
// desktop took the focus without covering anything, and the picture blinked
// off and on with every one of those clicks - a user's log had it hide and
// come back every one to two seconds. Nothing is covered by a click.
static bool CapturedWindowUnobstructed(const RECT &target)
{
    return !IsIconic(g_wgc_hwnd) && CapturedWindowCovered(target) < 0.25f;
}

// The same answer, held until it has been true for 200 ms. A menu or a
// notification sliding over the window for a moment must not blink the
// picture; a window the user really put there takes a fifth of a second.
// Minimising is deliberate and takes effect at once.
static bool CapturedWindowShouldShow(const RECT &target)
{
    static bool settled = true;
    static bool pending = true;
    static ULONGLONG pending_since = 0;
    if (IsIconic(g_wgc_hwnd)) { settled = pending = false; return false; }
    const bool now_ok = CapturedWindowUnobstructed(target);
    const ULONGLONG now = GetTickCount64();
    if (now_ok != pending) { pending = now_ok; pending_since = now; }
    if (pending != settled && now - pending_since >= 200) settled = pending;
    return settled;
}

static bool CapturedWindowRect(RECT &r)
{
    ...
    return SUCCEEDED(DwmGetWindowAttribute(g_wgc_hwnd, DWMWA_EXTENDED_FRAME_BOUNDS, &r, sizeof(r))) ||
...
static void FollowCapturedWindow()
{
    ...
    RECT r = {};
    if (!CapturedWindowRect(r)) return;
    if (!CapturedWindowShouldShow(r))
    {
        // Minimised or covered: the overlay gets out of the way instead of
        // hanging the picture over whatever is in front.
        if (g_present_shown)
        {
            ShowWindow(g_present_hwnd, SW_HIDE);
            g_present_shown = false;
            Log(IsIconic(g_wgc_hwnd) ? "[wgc] the window is minimised - the overlay is hidden"
                                     : "[wgc] another window covers it - the overlay is hidden");
        }
        return;
    }
...
//
// "Now and then" was every 300 frames - ten seconds and more at the rates the
// network runs at - so a game that stepped in front left the raw picture on
// screen that long, and the enhancement seemed to come and go. It is a clock
// now: every two seconds, and every ten while something keeps pushing back
// (five raises inside twenty seconds), so two windows that both insist on
// the top cannot turn into a fast blink.
//
// And only a window that covers at least half of the picture is raised over:
// that is a game gone full screen. The Start menu, a notification, the
// taskbar or a volume pop-up are smaller, belong in front, and are left
// there - raising over them faster would have hidden them.
static void ReassertPresentTopmost()
{
    if (g_wgc_active) return;
    if (g_present_hwnd == nullptr) return;
    static ULONGLONG last_check = 0, burst_start = 0;
    static int burst = 0;
    const ULONGLONG now = GetTickCount64();
    if (now - burst_start > 20000) { burst_start = now; burst = 0; }
    if (now - last_check < (burst >= 5 ? 10000u : 2000u)) return;
    last_check = now;
    HWND top = GetTopWindow(0);
    if (top == nullptr || top == g_present_hwnd) return;
    wchar_t overlayTitle[80] = {};
    if (GetWindowTextW(top, overlayTitle, 80) && wcscmp(overlayTitle, L"DLSS 5 MANAGER Screen overlay") == 0) return;
    wchar_t cls[64];
    ...
    if (GetClassNameW(top, cls, 64) > 0
        && (wcscmp(cls, L"pygame") == 0 || wcscmp(cls, L"DLSS5ManagerScreenPresent") == 0))
    ...
    // Invisible, cloaked or click-through windows cover nothing either -
    // raising over them would be a SetWindowPos for no reason.
    if (!IsWindowVisible(top)) return;
    BOOL cloaked = FALSE;
    if (SUCCEEDED(DwmGetWindowAttribute(top, DWMWA_CLOAKED, &cloaked, sizeof(cloaked))) && cloaked) return;
    const LONG_PTR ex = GetWindowLongPtrW(top, GWL_EXSTYLE);
    if ((ex & WS_EX_TRANSPARENT) != 0 && (ex & WS_EX_LAYERED) != 0) return;
    RECT mine = {}, both = {};
    if (!GetWindowRect(g_present_hwnd, &mine) || !IntersectRect(&both, &r, &mine)) return;
    const LONGLONG mine_area = static_cast<LONGLONG>(mine.right - mine.left) * (mine.bottom - mine.top);
    const LONGLONG over = static_cast<LONGLONG>(both.right - both.left) * (both.bottom - both.top);
    if (mine_area <= 0 || over * 2 < mine_area) return;
    SetWindowPos(g_present_hwnd, HWND_TOPMOST, 0, 0, 0, 0,
...
static void RevealOnFirstPresent()
{
    ...
    if (g_wgc_active && g_wgc_hwnd != nullptr)
    {
        RECT r = {};
        if (CapturedWindowRect(r) && !CapturedWindowUnobstructed(r)) return;
    }
    ShowWindow(g_present_hwnd, SW_SHOWNOACTIVATE);
    g_present_shown = true;
...
{
    if (g_present_swap != nullptr && g_present_w != 0 && FlowWanted() != g_present_on_flow_queue)
    {
        Log("[flow] FLOW MFG %s - the output window is rebuilt for it", FlowWanted() ? "on" : "off");
        g_present_stale = true;
    }
...
static bool PresentFrame(VideoState &v, UINT64 *submitted = nullptr)
{
    ...
    if (FlowActive())
    {
        // FLOW MFG's presenter shows it - real and in between - from its own
        // queue. A handover that fails still leaves the token the defer-tail
        // contract needs, and the next frame rebuilds the window without it.
        const bool handed = FlowPresent(v, v.output, D3D12_RESOURCE_STATE_UNORDERED_ACCESS) || FlowToken();
        if (submitted) *submitted = g_flow_present_fence;
        return handed;
    }
...
static bool PresentBypass(VideoState &v)
{
    if (!RebuildPresentIfStale()) return false;
    if (FlowActive())
        return FlowPresent(v, v.color.tex, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE) || FlowToken();
    // With the network off the presenter interpolates the raw capture, so the
    // doubled rate survives turning NR off. It used to be stopped here on
    // every frame, which is why the switch appeared to do nothing (the same
    // defect NeuralScreen fixed upstream in v1.16.0, #104).
    if (!g_hdr_capture && FgRequested() &&
        FgPresent(v, v.color.tex, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
        return true;
    StopFgPresentation();
...
//
// Halo reduction (gHalo > 0) changes two things, both in halo_reduction.inl's
// terms: the change comes from gDelta - the network's change with the wide
// bright/dark rims it draws along strong edges taken out - instead of
// nrOut - nrIn, and it is brought up to full size edge-aware: each of the four
// work-resolution samples counts in proportion to how much its input looks
// like this native pixel, so a change never lands on the wrong side of an
// edge. At gHalo == 0 it is exactly the composite above, value for value.
static const char kResidualHlsl[] =
    ...
    "Texture2D<float4>   gDelta  : register(t3);\n"
    "RWTexture2D<float4> gDst    : register(u0);\n"
    "cbuffer RC : register(b0) { uint gW; uint gH; float gStrength; float gHalo; };\n"
    ...
    "    {\n"
    "        uint sw, sh;\n"
    "        gNrIn.GetDimensions(sw, sh);\n"
    "        float2 p = uv * float2(sw, sh) - 0.5f;\n"
    "        float2 b = floor(p);\n"
    "        float2 f = p - b;\n"
    "        int2 a = int2(b);\n"
    "        int2 hi = int2(int(sw) - 1, int(sh) - 1);\n"
    "        float nl = dot(native, kLuma);\n"
    "        float3 num = 0.0f;\n"
    "        float den = 0.0f;\n"
    "        [unroll] for (int j = 0; j < 2; ++j)\n"
    "        [unroll] for (int i = 0; i < 2; ++i)\n"
    "        {\n"
    "            int2 q = clamp(a + int2(i, j), int2(0, 0), hi);\n"
    "            float wb = (i == 0 ? 1.0f - f.x : f.x) * (j == 0 ? 1.0f - f.y : f.y);\n"
    "            float dl = dot(gNrIn[q].rgb, kLuma) - nl;\n"
    "            float wr = exp(-dl * dl * 50.0f);\n"
    "            float w = wb * (1.0f - gHalo + gHalo * wr) + 1e-5f;\n"
    "            num += gDelta[q].rgb * w;\n"
    "            den += w;\n"
    "        }\n"
    "        delta = num / den;\n"
    "    }\n"
    "    float3 result = max(native + delta * gStrength, 0.0);\n"
...
static ID3D12Resource       *g_res_delta_bound;   // halo-corrected delta, or null
...
static bool EnsureScalePipeline()
{
    ...
    rres[0].NumDescriptors = 4;   // native, nr_in, nr_out, halo-corrected delta
    ...
    rres[1].OffsetInDescriptorsFromTableStart = 4;
    ...
    rparams[0].Constants.Num32BitValues = 4;   // gW, gH, gStrength, gHalo
...
static void BindResidualDescriptors(ID3D12Resource *native, ID3D12Resource *nr_in,
                                    ...
                                    ID3D12Resource *delta)
{
    if (native == g_res_native_bound && nr_in == g_res_in_bound &&
        nr_out == g_res_out_bound && dst == g_res_dst_bound && delta == g_res_delta_bound)
        return;
    const UINT stride = h.dev->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    // The halo-corrected delta is float (it is signed). Without halo reduction
    // there is none, and a null descriptor of the right shape stands in: the
    // shader never reads it then, but the table must still be complete.
    D3D12_SHADER_RESOURCE_VIEW_DESC dd = sd;
    dd.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
    h.dev->CreateShaderResourceView(delta, &dd, cpu);
    cpu.ptr += stride;
...
//
// delta/halo: the halo-reduced change and how strongly to use it - see
// halo_reduction.inl. nullptr/0 is the plain composite.
static void ResidualCompose(ID3D12Resource *native, ID3D12Resource *nr_in,
                            ...
                            ID3D12Resource *delta = nullptr, float halo = 0.0f)
{
    if (delta == nullptr) halo = 0.0f;
    BindResidualDescriptors(native, nr_in, nr_out, dst, delta);
    ...
    UINT32 rc[4] = { w, h_, 0, 0 };
    float str = strength;
    memcpy(&rc[2], &str, sizeof(float));
    memcpy(&rc[3], &halo, sizeof(float));
    h.list->SetComputeRoot32BitConstants(0, 4, rc, 0);
    D3D12_GPU_DESCRIPTOR_HANDLE gpu = g_residual_heap->GetGPUDescriptorHandleForHeapStart();
    h.list->SetComputeRootDescriptorTable(1, gpu);
    h.list->Dispatch((w + 7) / 8, (h_ + 7) / 8, 1);
}

#include "halo_reduction.inl"
#include "post_finish.inl"
...
static double                  g_capture_time = 0.0; // capture timestamp, seconds
...
static void CloseOfaGray();   // the flow's own luminance, defined below
...
// ---------------------------------------------------------------------------
// Gray downsample (GRAY): fractional area average RGBA -> R8 luminance.
// Each destination pixel integrates its exact source rectangle, including
// partial edge pixels, so non-integer ratios match cv2.resize(INTER_AREA). Bilinear sampling will NOT do: aliasing on text
...
static const char kGrayHlsl[] =
    ...
    "{\n"
    "    if (id.x >= gDstW || id.y >= gDstH) return;\n"
    "    float2 extent = float2(gSrcW, gSrcH) / float2(gDstW, gDstH);\n"
    "    float2 lo = float2(id.xy) * extent;\n"
    "    float2 hi = min(lo + extent, float2(gSrcW, gSrcH));\n"
    "    uint x0 = (uint)floor(lo.x), y0 = (uint)floor(lo.y);\n"
    "    uint x1 = min((uint)ceil(hi.x), gSrcW), y1 = min((uint)ceil(hi.y), gSrcH);\n"
    "    float sum = 0.0f;\n"
    "    for (uint y = y0; y < y1; ++y)\n"
    "        for (uint x = x0; x < x1; ++x)\n"
    "        {\n"
    "            float4 c = gSrc.Load(int3(x, y, 0));\n"
    "            float weight = max(0.0f, min(hi.x, x + 1.0f) - max(lo.x, (float)x))\n"
    "                         * max(0.0f, min(hi.y, y + 1.0f) - max(lo.y, (float)y));\n"
    "            sum += weight * dot(c.rgb, float3(0.299f, 0.587f, 0.114f));\n"
    "        }\n"
    "    gDst[id.xy] = sum / max((hi.x - lo.x) * (hi.y - lo.y), 1e-6f);\n"
    "}\n";

// The luminance the optical flow reads.
//
// The client's gray is 320x180 - enough for the scene-change score it is there
// for, and far too coarse to warp a face with: at that size a 4-pixel flow
// grid is one motion vector per 32x36 screen pixels at 1440p, and FLOW MFG
// showed it as smeared faces and torn edges. So the flow gets its own
// luminance, half the captured width (NS_OFA_SCALE divides it; 0 goes back to
// the client's gray), written by the same area kernel in the same command
// list. NVOFA then runs on that, with the finest grid the driver offers -
// typically 2, so 1280x720 in gives one vector per 2x2 of it: at 1440p that is
// a vector every 4 screen pixels instead of every 32.
static ID3D12Resource       *g_ofa_gray;
static ID3D12DescriptorHeap *g_ofa_gray_heap;
static UINT                  g_ofa_w, g_ofa_h;

static UINT OfaDivisor()
{
    static int cached = -1;
    if (cached < 0)
    {
        char buf[8] = {};
        const DWORD got = GetEnvironmentVariableA("NS_OFA_SCALE", buf, sizeof(buf));
        const int value = (got > 0 && got < sizeof(buf)) ? atoi(buf) : 2;
        cached = (value >= 0 && value <= 16) ? value : 2;
    }
    return static_cast<UINT>(cached);
}

static void CloseOfaGray()
{
    if (g_ofa_gray != nullptr) { g_ofa_gray->Release(); g_ofa_gray = nullptr; }
    if (g_ofa_gray_heap != nullptr) { g_ofa_gray_heap->Release(); g_ofa_gray_heap = nullptr; }
    g_ofa_w = g_ofa_h = 0;
}

static bool EnsureOfaGray(UINT capture_w, UINT capture_h)
{
    const UINT divisor = OfaDivisor();
    if (divisor == 0 || capture_w == 0 || capture_h == 0) return false;
    // Bound the expensive estimation to 720p; keep the original aspect ratio.
    // The finished colour/HUD are still warped at the captured resolution.
    const double scale = std::min({1.0 / divisor, 1280.0 / capture_w, 720.0 / capture_h});
    UINT w = std::max(4u, static_cast<UINT>(capture_w * scale) & ~3u);
    UINT hgt = std::max(4u, static_cast<UINT>(capture_h * scale) & ~3u);
    if (g_ofa_gray != nullptr && g_ofa_w == w && g_ofa_h == hgt) return true;
    CloseOfaGray();
    g_ofa_gray = MakeTex(w, hgt, DXGI_FORMAT_R8_UNORM, true);
    if (g_ofa_gray == nullptr) { Log("[ofa] %ux%u luminance failed", w, hgt); return false; }
    D3D12_DESCRIPTOR_HEAP_DESC hd = {};
    hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    hd.NumDescriptors = 2;
    hd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    if (FAILED(h.dev->CreateDescriptorHeap(&hd, __uuidof(ID3D12DescriptorHeap),
                                           reinterpret_cast<void **>(&g_ofa_gray_heap))))
    { CloseOfaGray(); return false; }
    // Created in COMMON, and put into UNORDERED_ACCESS - where it rests, like
    // the client's gray - on the caller's own open list. Opening a second one
    // here is what kept this from ever running: AreaToGray is already
    // recording, BeginCommands refuses, and the texture was built and thrown
    // away again every frame while the flow quietly stayed on the coarse gray.
    D3D12_RESOURCE_BARRIER to_uav = Transition(g_ofa_gray, D3D12_RESOURCE_STATE_COMMON,
                                               D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    h.list->ResourceBarrier(1, &to_uav);
    g_ofa_w = w;
    g_ofa_h = hgt;
    Log("[ofa] flow luminance %ux%u (capture %ux%u / %u)", w, hgt, capture_w, capture_h, divisor);
    return true;
}
...
// The seqlock has two writers while a recording is running: this answer, and
// the frames FLOW MFG shows in between (see PublishPresented). One at a time.
static std::mutex g_out_mutex;

// When a frame last arrived saying a recording is running (FRAME_FLAG_
// RECORDING). It is read on the presenter's thread, which has no frames of
// its own to count, so it is a moment rather than a flag: the recording is
// over when the frames stop saying so.
static std::atomic<uint64_t> g_recording_at{0};

static bool ExportingPresented()
{
    const uint64_t at = g_recording_at.load(std::memory_order_relaxed);
    return at != 0 && GetTickCount64() - at < 1500;
}

// A frame that was shown, into the section the client reads.
//
// FLOW MFG's frames in between never pass through the answer to a frame: they
// are made and shown on the presenter's own queue, so a recording of a
// multiplied picture used to hold the network's rate - the very thing the
// multiplication removes, missing from the recording of it. The presenter
// writes what it shows here instead, and the client picks it up on its own
// clock. `pitch` is the read back buffer's row stride, which D3D12 aligns.
static void PublishPresented(const BYTE *src, size_t pitch, UINT w, UINT hgt)
{
    const size_t row = static_cast<size_t>(w) * 4;
    std::lock_guard<std::mutex> lock(g_out_mutex);
    if (g_out_map == nullptr || src == nullptr) return;
    if (row * hgt + 8 != g_out_bytes) return;   // a different size: not ours
    uint64_t seq = ++g_out_seq;
    if ((seq & 1) == 0) ++seq;              // odd: writing
    memcpy(g_out_map, &seq, sizeof(seq));
    if (pitch == row)
        memcpy(g_out_map + 8, src, row * hgt);
    else
        for (UINT y = 0; y < hgt; ++y)
            memcpy(g_out_map + 8 + row * y, src + pitch * y, row);
    ++seq;                                  // even: done
    memcpy(g_out_map, &seq, sizeof(seq));
}
...
static bool AreaToGray()
{
    ...
    // The flow's own, finer luminance - same source, same kernel, its own
    // descriptors (a shader-visible heap is read when the dispatch runs, so
    // the two dispatches cannot share one pair).
    if (EnsureOfaGray(g_dda_w, g_dda_h))
    {
        D3D12_CPU_DESCRIPTOR_HANDLE ocpu = g_ofa_gray_heap->GetCPUDescriptorHandleForHeapStart();
        h.dev->CreateShaderResourceView(g_dda_dst, &sd, ocpu);
        D3D12_CPU_DESCRIPTOR_HANDLE ocpu2 = ocpu;
        ocpu2.ptr += stride;
        D3D12_UNORDERED_ACCESS_VIEW_DESC oud = {};
        oud.Format = DXGI_FORMAT_R8_UNORM;
        oud.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
        h.dev->CreateUnorderedAccessView(g_ofa_gray, nullptr, &oud, ocpu2);
        ID3D12DescriptorHeap *oheaps[] = { g_ofa_gray_heap };
        h.list->SetDescriptorHeaps(1, oheaps);
        const UINT osizes[4] = { g_dda_w, g_dda_h, g_ofa_w, g_ofa_h };
        h.list->SetComputeRoot32BitConstants(0, 4, osizes, 0);
        D3D12_GPU_DESCRIPTOR_HANDLE og0 = g_ofa_gray_heap->GetGPUDescriptorHandleForHeapStart();
        D3D12_GPU_DESCRIPTOR_HANDLE og1 = og0;
        og1.ptr += stride;
        h.list->SetComputeRootDescriptorTable(1, og0);
        h.list->SetComputeRootDescriptorTable(2, og1);
        h.list->Dispatch((g_ofa_w + 7) / 8, (g_ofa_h + 7) / 8, 1);
        h.list->SetDescriptorHeaps(1, heaps);
    }
...
static bool OpenDda(UINT w, UINT hgt)
{
    ...
    // An HDR desktop is always duplicated in FP16, whatever the HDR switch
    // says. The legacy 8-bit duplication of an HDR desktop clips everything
    // above 80 nits - at a 200-nit SDR white that is most of the picture -
    // and that clipped copy was what the network got and what people saw:
    // the washed-out, too-bright LIVE FLOW HDR users reported. The switch
    // still decides the presentation (g_hdr_capture); with it off the FP16
    // frame is tone-mapped to SDR properly in the capture shader.
    g_dda_hdr_mode = g_capture_display.enabled;
                                      ...
                                      DXGI_FORMAT *out_format = nullptr)
{
    ...
    if (g_dda_shared == nullptr)
    {
        ...
        // and only on a display that is really in HDR: a 10-bit SDR desktop
        // hands back FP16 too, and presenting that as scRGB drew the dim
        // network proxy, not the picture.
        g_hdr_capture = g_capture_float && HdrEnabled() && g_capture_display.enabled;
...
static bool SwizzleCaptureIntoColor(VideoState &v)
{
    ...
    // 1: the HDR composite's proxy; 2: the SDR picture itself (see
    // kHdrCaptureHlsl).
    struct { UINT is_float; float white; UINT rotate180; } hdr = {
        g_capture_float ? (g_hdr_capture ? 1u : 2u) : 0u, g_hdr_frame_white,
...
{
    CaptureGuard guard;
    if (g_snapshot_frozen) return false;
    ...
    if (GetTickCount64() - last_mode_query > 1000)
    {
        ...
        if (g_dda_hdr_mode != g_capture_display.enabled)
        {
    ...
    if (g_capture_visual_changed) {
        ...
        g_capture_time = double(fi.LastPresentTime.QuadPart) / frequency.QuadPart;
    }
...
{
    std::mutex mutex;
    ns_wgc::Direct3D11CaptureFrame latest{nullptr};
    winrt::handle arrived{CreateEventW(nullptr, FALSE, FALSE, nullptr)};
    std::atomic<bool> latest_only{false};
    ~WgcMailbox() { try { if (latest) latest.Close(); } catch (...) {} }
};
...
struct WgcSession
{
    ...
    bool hdr = false;
    std::shared_ptr<WgcMailbox> mailbox;
    winrt::event_token arrival_token{};
};
...
static void CloseWgc()
{
    ...
    if (g_wgc != nullptr)
    {
        try
        {
            if (g_wgc->pool != nullptr && g_wgc->arrival_token.value)
                g_wgc->pool.FrameArrived(g_wgc->arrival_token);
...
static bool OpenWgc(HWND hwnd)
{
    ...
    try
    {
        ...
        g_capture_display = QueryHdrDisplay(g_capture_monitor);
        // As for duplication: FP16 whenever the display is HDR, so the
        // window is never clipped to 8 bits before anything sees it.
        s->hdr = g_capture_display.enabled;
        ...
        s->mailbox = std::make_shared<WgcMailbox>();
        s->mailbox->latest_only = FlowWanted();
        if (s->mailbox->arrived) {
            // The callback holds the handle alive even if capture closes
            // concurrently. Never capture the raw WgcSession pointer here.
            s->arrival_token = s->pool.FrameArrived([box = s->mailbox](auto const &pool, auto const &) {
                // Drain while NR is running too. A two-frame WGC pool left
                // unread during a long evaluate fills with OLD images; a
                // drain on the render thread alone cannot fix that backlog.
                try {
                    if (box->latest_only) {
                        ...
                        for (int n = 0; n < 8; ++n) {
                            ...
                            if (!frame) break;
                            if (box->latest) box->latest.Close();
                            box->latest = frame;
                        }
                    }
                    SetEvent(box->arrived.get());
                } catch (winrt::hresult_error const &) { /* pool is closing */ }
            });
        }
...
static bool WgcGrab(VideoState &v)
{
    ...
    if (monitor != g_capture_monitor || GetTickCount64() - last_mode_query > 1000)
    {
        ...
        last_mode_query = GetTickCount64();
        if (g_wgc->hdr != g_capture_display.enabled)
        {
    ...
    {
        const double t_acq = PhaseNow();
        auto box = g_wgc->mailbox;
        box->latest_only = FlowWanted();
        auto take_latest = [&]() {
            std::lock_guard<std::mutex> lock(box->mutex);
            auto frame = box->latest;
            box->latest = nullptr;
            // Also drain on a mode switch, before another callback arrives.
            for (int n = 0; n < 8; ++n) {
        ...
        auto frame = take_latest();
        // Wake on actual captured content. Sleep(1) polling can delay the
        // next input by a Windows timer quantum (~15ms), even with NR off.
        // Only MFG opts in; a bounded wait keeps commands/closing responsive.
        if (frame == nullptr && FlowWanted() && box->arrived) {
            WaitForSingleObject(box->arrived.get(), 8);
            frame = take_latest();
        }
        ...
        g_capture_time = frame.SystemRelativeTime().count() * 1e-7;
...
#include "multipass.inl"

// The before/after wipe for the frame being evaluated, 0 when there is none.
// Set by the frame loop just before EvaluateVideo and recorded at the end of
// the same command list (RecordSplit): the wipe used to be a submission of
// its own with a wait on it, which took every compared frame off the
// deferred path - the network, the wipe and the present ran one after the
// other instead of overlapping, and the picture slowed down exactly while
// someone was dragging the divider to judge it.
static UINT g_eval_split_x = 0;
static void RecordSplit(VideoState &v, UINT split_x);

static bool EvaluateVideo(VideoState &v, int reset, UINT64 *submitted = nullptr)
{
    if (!EnsureMultiPass(v)) return false;
    if (submitted) *submitted = 0;
    if (!BeginCommands()) return false;
    ...
    // Halo reduction with the network at full resolution (Enhance off): the
    // network writes a scratch texture, and the composite below turns it
    // into the output. Otherwise it writes the output directly, as always.
    const float halo = CurrentHalo();
    const bool halo_full = !v.nr_small && halo > 0.0f && EnsureHaloPipeline() &&
                           EnsureHaloTargets(cw, ch, true);
    ID3D12Resource *nr_result = v.nr_small ? v.nr_out : (halo_full ? g_halo_t.raw : v.output);
    h.params->Reset();
    ...
    if (code == 0 && !NVSDK_NGX_FAILED(result) && g_pass_count > 1)
        result = EvaluateExtraLayers(nr_result, reset);
    ...
    if (v.nr_small)
    {
        ...
        D3D12_RESOURCE_BARRIER to_srv = Transition(
            ...
            D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        h.list->ResourceBarrier(1, &to_srv);
        const bool halo_small = v.residual && halo > 0.0f && EnsureHaloPipeline() &&
                                EnsureHaloTargets(nw, nh, false);
        if (halo_small)
        {
            HaloFilter(v.nr_in, v.nr_out, nw, nh, halo, v.mv.tex, v.w, v.hgt);
            ResidualCompose(v.color.tex, v.nr_in, v.nr_out, cw, ch, v.output,
                            v.residual_strength, g_halo_t.dc, halo);
            HaloDone();
        }
        else if (v.residual)
            ResidualCompose(v.color.tex, v.nr_in, v.nr_out, cw, ch, v.output,
    ...
    {
        // Full resolution: the input is the native frame itself, so it is
        // native, nr_in and the edge guide at once, and nothing is scaled.
        D3D12_RESOURCE_BARRIER to_srv = Transition(
            g_halo_t.raw, D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        h.list->ResourceBarrier(1, &to_srv);
        HaloFilter(v.color.tex, g_halo_t.raw, cw, ch, halo, v.mv.tex, v.w, v.hgt);
        ResidualCompose(v.color.tex, v.color.tex, g_halo_t.raw, cw, ch, v.output,
                        1.0f, g_halo_t.dc, halo);
        HaloDone();
        D3D12_RESOURCE_BARRIER back = Transition(
            g_halo_t.raw, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        h.list->ResourceBarrier(1, &back);
    }
    // Sharpness, edge smoothing and the colours, on the finished picture -
    // not recorded at all while all of them are at rest (post_finish.inl).
    const float sharp = CurrentSharpness(), edge = CurrentEdgeSmooth();
    if ((sharp > 0.0f || edge > 0.0f || ColorsActive()) && EnsureFinishPipeline() &&
        EnsureFinishTargets(cw, ch))
        FinishOutput(v.output, cw, ch, sharp, edge, v.mv.tex, v.w, v.hgt, reset != 0);
    else
        FinishSkipped();
    // The wipe goes last, over everything above: its left side is the
    // capture as it came in, colours and all left out, which is the point.
    if (g_eval_split_x != 0)
    {
        RecordSplit(v, g_eval_split_x);
        g_eval_split_x = 0;
    }
...
// internally), so this is a single region copy, entirely on the GPU. It is
// recorded into the evaluate list (EvaluateVideo, g_eval_split_x), so it runs
// BEFORE the present and before the pixels are handed over, and the wipe ends
// up in the recording and the screenshot by itself. It needs the list open
// and v.output in UNORDERED_ACCESS, and leaves both as it found them.
static void RecordSplit(VideoState &v, UINT split_x)
{
    const UINT cw = v.upscale ? v.full_w : v.w;
    const UINT ch = v.upscale ? v.full_h : v.hgt;
    if (split_x == 0 || cw == 0 || ch == 0) return;
    ...
    if (split_x < cw && EnsureSplitUav(v.output))
    {
        ...
        // #76B900 - Manager green comparison divider. The texture is plain
        // UNORM (not _SRGB), so the bytes go in as they are, without a gamma
        // conversion.
        const FLOAT accent[4] = { 118.0f / 255.0f, 185.0f / 255.0f, 0.0f / 255.0f, 1.0f };
                            ...
                            const BYTE **color_ptr, const BYTE **mv_ptr)
{
    ...
    {
        // Keep the door open for the presenter's export while the client is
        // recording (see ExportingPresented).
        if ((fh.reserved & FRAME_FLAG_RECORDING) != 0)
            g_recording_at.store(GetTickCount64(), std::memory_order_relaxed);
    ...
    for (;;)
    {
        ...
        if (msg == 2)
        {
            ...
            if (same_size && h.feature != nullptr)
            {
                ...
                HaloFromResizeFlags(rc.flags);
                ExtrasFromResize(rc.profile);
                ColorsFromResize(rc.flags, rc.preset);
        ...
        if ((msg == 1 || msg == 10) && g_snapshot_frozen && g_present_hwnd) {
            prepared = (msg == 10);
            prepared_index = fh.index;
            prepared_got = false;
            Sleep(15);
            VideoResultHeader frozen = { OUT_MAGIC, fh.index, 1u, 0u, g_last_eval_result, fh.pts };
            if (!WriteExact(g_wire, &frozen, sizeof(frozen))) return 10;
            continue;
        }
        if (msg == 10)
        {
        ...
        if (CaptureActive())
        {
            ...
            if (!got)
            {
                ...
                const bool out_changed = want_bypass != g_last_out_bypass ||
                                         ...
                                         g_force_next_frame;
                // Re-submitting a cached capture makes OFA compare the image
                // with itself and replaces the real pair with zero motion.
                // MFG must advance history only when capture supplies a frame.
                const bool skip = ((fh.reserved & FRAME_FLAG_SKIP_STATIC) != 0 || FlowWanted()) &&
                                  (fh.reserved & FRAME_FLAG_WANT_PIXELS) == 0 &&
                                  !out_changed;
                if (skip)
                {
                    ...
                    // WGC waits on FrameArrived; desktop duplication already
                    // waits on AcquireNextFrame. No extra polling sleep.
                    continue;
                }
            }
            ...
            defer_tail = !g_hdr_capture && warmup_done && h.feature != nullptr &&
                PresentModeActive(v) &&
                (fh.reserved & (FRAME_FLAG_BYPASS | FRAME_FLAG_WANT_PIXELS)) == 0;
        ...
        g_fg_reset = frame == 0 || fh.reset != 0 || bypass != g_last_bypass
                     || previous_hdr_split != g_hdr_split
                     || (stall_pending && source_fresh);
        g_last_bypass = bypass;
        ...
        if (!bypass)
        {
            ...
            g_eval_split_x = (fh.reserved & FRAME_FLAG_SPLIT) != 0
                ? SplitXFromFlags(fh.reserved, v.upscale ? v.full_w : v.w) : 0u;
...
static void CleanupVideoNgx()
{
    CloseFlow(); // retire presentation before tearing down the GPU engines/export.
    CloseNvofa();
    CloseFgResources();
    ReleaseMultiPass();
    if (h.feature != nullptr)
