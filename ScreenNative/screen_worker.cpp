...
// A captured app must never pin its image over an unrelated foreground app.
static bool CapturedWindowForeground()
{
    if (!g_wgc_active || !g_wgc_hwnd) return true;
    HWND foreground = GetForegroundWindow();
    wchar_t title[80] = {};
    if (foreground && GetWindowTextW(foreground, title, 80) &&
        wcscmp(title, L"DLSS 5 MANAGER Screen overlay") == 0) return true;
    return foreground && GetAncestor(foreground, GA_ROOTOWNER) ==
                         GetAncestor(g_wgc_hwnd, GA_ROOTOWNER);
}
...
static void FollowCapturedWindow()
{
    ...
    if (IsIconic(g_wgc_hwnd) || !CapturedWindowForeground())
    {
...
static void RevealOnFirstPresent()
{
    ...
    if (!CapturedWindowForeground()) return;
    ShowWindow(g_present_hwnd, SW_SHOWNOACTIVATE);
    g_present_shown = true;
