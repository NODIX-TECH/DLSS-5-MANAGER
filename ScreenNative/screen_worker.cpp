...
static bool EnsureOfaGray(UINT capture_w, UINT capture_h)
{
    ...
    UINT w = std::min(1280u, std::max(512u, (capture_w / divisor) & ~3u));
    UINT hgt = std::min(720u, std::max(288u, (capture_h / divisor) & ~3u));
    if (w >= capture_w || hgt >= capture_h) { w = capture_w & ~3u; hgt = capture_h & ~3u; }
...
static bool EvaluateVideo(VideoState &v, int reset, UINT64 *submitted = nullptr)
{
    ...
    if (v.nr_small)
    {
        ...
        if (halo_small)
        {
            HaloFilter(v.nr_in, v.nr_out, nw, nh, halo);
    ...
    else if (halo_full)
    {
        ...
        D3D12_RESOURCE_BARRIER to_srv = Transition(
            ...
            D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        h.list->ResourceBarrier(1, &to_srv);
        HaloFilter(v.color.tex, g_halo_t.raw, cw, ch, halo);
    ...
    // Sharpness and edge smoothing, on the finished picture - not recorded at
    // all while both are 0 (post_finish.inl).
    const float sharp = CurrentSharpness(), edge = CurrentEdgeSmooth();
    if ((sharp > 0.0f || edge > 0.0f) && EnsureFinishPipeline() && EnsureFinishTargets(cw, ch))
        FinishOutput(v.output, cw, ch, sharp, edge);
