...
type PulseMessageViewModel
    ...
    /// Fixing your own words, for one minute after sending.
    ///
    /// The server enforces the same window; this only decides whether the link
    /// is drawn at all, so the option disappears from the bar rather than
    /// failing under the pointer.
    member _.CanEditNow =
        mine
        && not isPending
