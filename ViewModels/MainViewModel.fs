...
type MainViewModel() as this =
    ...
    /// Backgrounds are static for all atmosphere and motif presets.
    ...
    /// Arms the countdown, unless this machine has turned automatic scanning
    /// off - in which case nothing happens at all until the user asks.
    member this.ArmAutoScan() =
        if GameScanner.isAutoScanDisabled () then
    ...
    /// "Not now" - this launch only. The next one asks again.
    ...
    /// "Never" - and it stays off until it is turned back on in Settings.
    member this.DisableAutoScan() =
        GameScanner.setAutoScanDisabled true
        this.CancelAutoScan()
        this.ScanStatusText <- "Automatic scan is off"
    ...
    member this.IsAutoScanEnabled
        with get () = not (GameScanner.isAutoScanDisabled ())
        and set value =
            GameScanner.setAutoScanDisabled (not value)
            this.RaisePropertyChanged("IsAutoScanEnabled")
    ...
    member this.DeepFriedHint =
        if this.IsDeepFriedHost64 then
            "May improve compatibility on this route - leaving it on is recommended"
        elif this.IsVulkanMode then
            "Installs deep-fried chicken beside the game - Vulkan has no RenoDX, so this is the effect itself"
        else
            "Installs deep-fried chicken instead of RenoDX, and switches Multipass off"
    ...
    member this.InstallModeHintText =
        ...
        | ModInstaller.VulkanMode ->
            "ReShade on the Vulkan layer, with the Vulkan payload in RenoDX's place."
