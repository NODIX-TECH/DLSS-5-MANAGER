...
type ScreenViewModel() as this =
    ...
    let passOptions = [| "1 · Standard"; "2 · Multi-pass"; "3 · Multi-pass" |]
    ...
    let sliderKeys =
        Set.ofList [ "passes"; "work_scale"; "intensity"; "local_tone"; "local_structure"; "skin_structure"; "split" ]
    ...
    let numberProperties =
        dict [ "passes", [| "PassCount"; "PassIndex" |]
    ...
    member _.PassCount with get () = float(this.PassIndex + 1) and set (v: float) = setNumber "passes" (Math.Clamp(Math.Round(v), 1., 3.))
    ...
    member _.ApplyQuery(value: string) =
        ...
        notify ()
    member _.ShowProcessing = matches "processing dlss 5 nr dlss 4.5 fg frame generation multiplier boost network resolution fps multipass multi-pass passes معالجة دقة توليد اطارات تمريرات ميلتي باس"
    ...
    member _.PassIndex with get () = int (number "passes" 1.0) - 1 and set v = if v >= 0 && v <= 2 then setNumber "passes" (float (v + 1))
